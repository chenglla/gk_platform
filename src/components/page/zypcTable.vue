    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 志愿批次数据</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<!--                </el-select>-->
                ...
                <el-button type="primary" icon="search" @click="searchOneScore" class="search_pc">搜索</el-button>
...
<!--&lt;!&ndash;                <el-table-column label="年份" sortable width="120">&ndash;&gt;-->
<!--                    <template slot-scope="scope">-->
...
<!--                <el-table-column label="分数区间上" sortable width="120">-->
                    <template slot-scope="scope">
                        <span>{{scope.row.batchinfo}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="label" label="志愿类型">
<!--                <el-table-column prop="label" label="分数区间下" width="120">-->
                    <template slot-scope="scope">
                        <span>{{scope.row.parallel}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="label" label="填报学校数">
<!--                <el-table-column prop="label" label="区间人数" width="120">-->
<!--                <el-table-column prop="url" label="区间人数" :formatter="formatter">-->
                    <template slot-scope="scope">
                        <span>{{scope.row.schoolNum}}</span>
<!--                        <a :href="scope.row.subscore" target="_blank" class="a_content">{{scope.row.supscore}}</a>-->
                        <!--                        <span>{{scope.row.fields.url}}</span>-->
                    </template>
                </el-table-column>
                <el-table-column prop="label" label="填报专业数">
<!--                <el-table-column prop="label" label="累计排名" width="120" fit>-->
                    <template slot-scope="scope">
                        <span>{{scope.row.majorNum}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="label" label="可否调剂">
<!--                <el-table-column prop="label" label="累计排名" width="120" fit>-->
                    <template slot-scope="scope">
                        <span v-if="scope.row.adjust === 1">可调剂</span>
                        <span v-if="scope.row.adjust === 0">不可调剂</span>
                    </template>
                </el-table-column>
...
<!--            <el-form ref="form" :model="form" label-width="50px">-->
                ...
                <el-form-item label="类别" label-width="100px">
                    <el-input v-model="form.type" class="input_width_info"></el-input>
                </el-form-item>
                <el-form-item label="批次信息" label-width="100px">
                    <el-input v-model="form.batchInfo" class="input_width_info"></el-input>
                </el-form-item>
                <el-form-item label="志愿类型" label-width="100px">
                    <el-input v-model="form.parallel" class="input_width_info"></el-input>
                </el-form-item>
                <el-form-item label="填报学校数" label-width="100px">
                    <el-input v-model="form.schoolNum" class="input_width_info"></el-input>
                </el-form-item>
                <el-form-item label="填报专业数" label-width="100px">
                    <el-input v-model="form.majorNum" class="input_width_info"></el-input>
                </el-form-item>
                <el-form-item label="可否调剂" label-width="100px">
                    <el-input v-model="form.adjust" class="input_width_info"></el-input>
                    <div class="item_tip">0表示不可调剂，1表示可调剂</div>
                </el-form-item>
...
<script>
    ...
    import {getOneScoreProvince, updateBatchInfo, getzyBatchInfo, updateOneScore} from '@/api/index'
    ...
    export default {
        ...
        methods: {
            ...
            handleEdit(index, row) {
                ...
                this.form.batchInfo = row.batchinfo
                this.form.parallel = row.parallel
                this.form.schoolNum = row.schoolNum
                this.form.majorNum = row.majorNum
                this.form.adjust = row.adjust
            ...
            saveEdit () {
                if (this.title === '添加') {
                    updateBatchInfo({
                        city: this.form.city,
                        type: this.form.type,
                        batchinfo: this.form.batchInfo,
                        parallel: this.form.parallel,
                        schoolNum: this.form.schoolNum,
                        majorNum: this.form.majorNum,
                        adjust: this.form.adjust
                    }).then(res => {
                        if (res.data.code === 0) {
